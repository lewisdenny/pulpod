...
// TODO: Extract container spec
func (p PodmanManager) CreateContainer(containerImage string) (string, error) {
	...
	return createResponse.ID, err // TODO: Should we return nil or err, maybe they are the same.
}

// StartContainer starts a created Podman container.
func (p PodmanManager) StartContainer(containerID string) error {
	...
	if err := containers.Start(p.clientCtx, containerID, nil); err != nil {
		logging.Log.Errorf("Failed to create Podman container: %s\n", containerID)
		return err
	}
